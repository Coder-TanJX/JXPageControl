...
@IBDesignable public class JXPageControlScale: UIView, JXPageControlType {
    ...
    /// Active item size
    @IBInspectable var activeSize: CGSize = CGSize(width: 15,
                                                   height: 15) {
        didSet {
            reloadLayout()
...
//                }
    ...
    func hollowLayout(layer: CALayer, isActive: Bool) {
        /// Set backgroundcolor
        if isInactiveHollow {
            if isActive,
                !isActiveHollow {
